...
public class TagElevatorTests
{
    ...
    public void AreSequenceTagsAmbiguous()
    {
        ...
        foreach (var entry in DicomDictionary.Default)
        {
            ...
            if (entry.ValueRepresentations.Any(static v => v == DicomVR.SQ))
                Assert.That(entry.ValueRepresentations, Has.Length.EqualTo(1), $"Failed on {entry.Name}"); //can they ONLY be sequences (pretty please!)
        }
    }
    [Test]
    ...
    public void TagElevation_TwoDeep_FindValue()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(value, Is.EqualTo("KESPR LUT"));
        });
    }

    //tests reading a non existent tag
    ...
    public void TagElevation_TwoDeepWithConditional_FindValue()
    {
        ...
        {
            Assert.That(file, Is.Not.Null);
            Assert.That(value, Is.EqualTo("Redlands Clinic"));
        });
    }
    ...
    public void TagElevation_Multiplicity(bool specifyConcatenateMultiplicity)
    {
        ...
        if (specifyConcatenateMultiplicity)
        {
            ...
            Assert.That(elevator.GetValue(ds), Is.EqualTo("abcd\\efgh"));
        }
        else
        {
            ...
            Assert.That(ex.Message, Does.Contain("Multiplicity"));
        }
    }
    ...
    public void TagElevation_Multiplicity_Conditional(string toFind, string expectedResults)
    {
        ...
        Assert.That(elevator.GetValue(ds), Is.EqualTo(expectedResults));
    }
    ...
    public void TagElevation_MultipleLeavesAndMultiplicity_FindValues()
    {
        ...
        //should pass when you do
        Assert.That(elevator.GetValue(ds), Is.EqualTo($"abcd\\efgh{Environment.NewLine}hij\\klm"));
    }
    ...
    public void TagElevation_MultipleLeavesAndMultiplicity_WithArraySiblingConditional_FindValues()
    {
        ...
        //should pass when you do
        Assert.That(elevator.GetValue(ds), Is.EqualTo("Description"));
    }
    ...
    public void TagElevation_ConditionalCurrentNode(string conditional, string conditionalMatch, bool expectedToFind)
    {
        ...
        if (expectedToFind)
            Assert.That(value, Is.EqualTo("Treatment"));
        else
            Assert.That(value, Is.Null);
    }
    ...
    public void TagElevation_DoubleDotConditional_ThenArrayElementBuddies_FindValue(string conditional, string conditionalMatch)
    {
        ...
        Assert.That(value, Is.EqualTo($"Observer Name{Environment.NewLine}Observer Organization Name{Environment.NewLine}Description{Environment.NewLine}Diagnosis{Environment.NewLine}Treatment"));
    }
    ...
    public void TagElevation_SiblingConditionals(string conditional, string conditionalMatch, string expectedResults)
    {
        ...
        Assert.That(value, Is.EqualTo(expectedResults));
