...
public static class DicomTypeTranslater
{
    ...
    /// <returns></returns>
    public static DatabaseTypeRequest GetNaturalTypeForVr(IEnumerable<DicomVR> valueRepresentations, DicomVM valueMultiplicity)
    {
    ...
    private static DatabaseTypeRequest Conflate(List<DatabaseTypeRequest> vrs)
    {
        ...
        foreach (var newType in vrs)
        {
            ...
            if (toReturn.CSharpType != newType.CSharpType)
                if (
                    (toReturn.CSharpType == typeof(ushort) ||
                     toReturn.CSharpType == typeof(short)) //some tags e.g. SmallestValidPixelValue can be either ushort or short 
                    && newType.CSharpType == typeof(ushort) || newType.CSharpType == typeof(short))
                    t = typeof(int); //if they are being all creepy about it just use an int that way there's definitely enough space
                else
